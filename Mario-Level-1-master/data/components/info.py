...
class OverheadInfo(object):
    ...
    def create_main_menu_labels(self):
        ...
        self.create_label(player_one_game, 'LOOK AT THE', 272, 360)
        self.create_label(player_two_game, 'CONTROLLER', 272, 405)
        self.create_label(top, 'TOP - ', 280, 465)
    ...
    def handle_level_state(self, level_info):
        ...
        if self.state == c.MAIN_MENU:
            ...
            if level_info[c.LEVEL_STATE] != c.FROZEN \
                    and level_info[c.LEVEL_STATE] != c.PAUSE \
    ...
    def draw(self, surface, game_info = 0):
        ...
        if self.state == c.MAIN_MENU:
            ...
            self.draw_level_screen_info(surface,game_info)
        elif self.state == c.GAME_OVER:
    ...
    def draw_level_screen_info(self, surface, game_info):
        ...
        for label in self.label_list:
            for letter in label:
                surface.blit(letter.image, letter.rect)
        if not (type(game_info) is int):
            if (game_info[c.LEVEL_STATE] == c.PAUSE):
                for letter in  self.main_menu_labels[0]:
                    surface.blit(letter.image, letter.rect)
                for letter in  self.main_menu_labels[1]:
                    surface.blit(letter.image, letter.rect)
                pause = []
                self.create_label(pause,"PAUSE",340, 250)
                for letter in  pause:
                    surface.blit(letter.image, letter.rect)
